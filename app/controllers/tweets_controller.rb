...
def search
  if Rails.env.production?
    @tweets = Tweet.where("description ILIKE ?", "%#{params[:q]}%").paginate(page: params[:page], per_page: 10)
  else
    @tweets = Tweet.where("description LIKE ?", "%#{params[:q]}%").paginate(page: params[:page], per_page: 10)
  end
  render :index
end
